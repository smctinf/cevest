...
def GerarCertificados(request):
    ...
    if (cpf):
        ...
        if (aluno):
            turma_aluno = Aluno_Turma.objects.filter(
                turma=turma, situacao=aprovado.id, aluno=aluno)
        else:
            turma_aluno = []
    else:
        turma_aluno = Aluno_Turma.objects.filter(
            turma=turma, situacao=aprovado.id)
    ...
    matrizes = Matriz.objects.filter(
        curso=curso_turma, curriculo=turma.curriculo)
    disciplinas = []
    total_horas = 0
    total_aulas = 0

    for mat in matrizes:
        disciplinas.append(temp_disciplina(
            mat.disciplina.nome, mat.num_aulas, mat.carga_horaria_total))
    ...
    if request.method == 'POST':
        ...
        if temp_dia.is_valid():
            ...
            if index_data == len(choices)-1:
                ...
                request.session['data'] = convert_date_to_tuple(
                    [dias_aula[index_data], ])
...
def ControleDePresenca(request):
    ...
    for ta in turma_aluno:
        presenca = []
        temp_presenca = Presenca.objects.filter(
            turma=temp_turma, aluno=ta.aluno)
    ...
    # cria o form
    formset_controle_presenca = formset_factory(
        Controle_Presenca, max_num=len(turma_aluno))
    formset = formset_controle_presenca(
        form_kwargs={'CHOICES': choices}, initial=initial_data)

    # Validação e salvar
    if request.method != 'POST':
        return render(request, "Administracao/controle_frequencia.html", {"formset": formset, "data": dias_aula})
    temp_presenca = formset_controle_presenca(
        request.POST, form_kwargs={'CHOICES': choices}, initial=initial_data)
        ...
        for form in temp_presenca:
            ...
            for index in range(0, len(dias_aula)):
                if str(index) in dias:
                    temp_presenca_criada, created_presenca = Presenca.objects.get_or_create(
                        turma=temp_turma, aluno=aluno, data_aula=dias_aula[index])
                    ...
                    if Presenca.objects.filter(turma=temp_turma, aluno=aluno, data_aula=dias_aula[index], presente=True).count() > 0:
                        temp_presenca_criada = Presenca.objects.get(
                            turma=temp_turma, aluno=aluno, data_aula=dias_aula[index], presente=True)
...
def ArrumarSituacaoTurmaBacalhau(request):
    ...
    messages.info(
        request, 'Situação de todas as turmas previstas mudada para confirmada')
...
def ConfirmarTurma(request):
    ...
    temp_status_aluno = Status_Aluno_Turma_Prevista.objects.get(
    ...
    formset_confirmar_turma = formset_factory(
        Confirmar_Turma, max_num=len(temp_turmas))
    formset = formset_confirmar_turma(initial=initial_data)

    if request.method == 'POST':
        temp_turmas_confirmadas = formset_confirmar_turma(
        ...
        if temp_turmas_confirmadas.is_valid():
            ...
            for form in temp_turmas_confirmadas:
                if form.cleaned_data['confirma']:
                    ...
                    if created_turma:
                        temp_turma_criada.horario.set(
                            temp_turmas[i].horario.all())
                        temp_aluno_turma_prevista = Aluno_Turma_Prevista.objects.filter(
                            turma_prevista=temp_turmas[i], status_aluno_turma_prevista=temp_status_aluno)
                        for aluno_turma_relacao in temp_aluno_turma_prevista:
                            temp_aluno_turma = Aluno_Turma.objects.create(
                                turma=temp_turma_criada, aluno=aluno_turma_relacao.aluno)
...
def EscolherTurmaPrevistaParaAlocacao(request):
    ...
    turmas_previstas = Turma_Prevista.objects.exclude(
        situacao=situacao_cancelada)
    turmas_previstas = turmas_previstas.exclude(
        dt_fim__lt=datetime.date.today())
...
def Alocacao(request):
    ...
    turma_prevista_alunos = Aluno_Turma_Prevista.objects.filter(
    ...
    for tmp_turma_prevista_alunos in turma_prevista_alunos:
        alunos_compativeis = alunos_compativeis.exclude(
            id=tmp_turma_prevista_alunos.aluno_id)

    status_candidato = Status_Aluno_Turma_Prevista.objects.get(
        descricao="Candidato")
    status_matriculado = Status_Aluno_Turma_Prevista.objects.get(
        descricao="Matriculado")

    # exclui alunos já matriculados ou já selecionados, sobrando os que desistiram da vaga ou não compareceram

    turma_prevista_alunos = turma_prevista_alunos.exclude(
        ...
        for turma_aluno in aluno_turma:
            ...
            for horario in turma_aluno.turma.horario.all():
                if horario in turma_prevista.horario.all():
                    print(
                        "aluno removido da lista por conflito de horário:" + str(aluno))
                    alunos_compativeis = alunos_compativeis.exclude(
                        id=aluno.id)
    ...
    for aluno in alunos_compativeis:
        ...
        aluno_turma_prevista_temp = Aluno_Turma_Prevista.objects.filter(
            aluno=aluno)
        ...
        temp_dict = {"aluno": aluno, "pontuação": temp_pontuacao, "dt_nasc": aluno.dt_nascimento,
                     'dt_inclusao': aluno.dt_inclusao, 'situacao': Status_Aluno_Turma_Prevista.objects.none()}
    ...
    lista_final = sorted(
        lista_final, key=lambda i: (-i['pontuação'], i['dt_inclusao'], i['dt_nasc']))

    quant_na_turma = Aluno_Turma_Prevista.objects.filter(turma_prevista=turma_prevista).filter(
        status_aluno_turma_prevista=status_candidato).count()
    quant_na_turma += Aluno_Turma_Prevista.objects.filter(turma_prevista=turma_prevista).filter(
        status_aluno_turma_prevista=status_matriculado).count()
...
def AlterarSituacaoTurmaPrevista(request):
    situacao_matriculado = Status_Aluno_Turma_Prevista.objects.get(
        descricao="Matriculado")
    situacoes_nao_matriculados = Status_Aluno_Turma_Prevista.objects.exclude(
        descricao="Matriculado")
    ...
    situacao_candidato = Status_Aluno_Turma_Prevista.objects.get(
        descricao="Candidato")

    turma_prevista_id = request.session['turma_prevista_id']
    turma_prevista = Turma_Prevista.objects.get(id=turma_prevista_id)
    aluno_turma_prevista = Aluno_Turma_Prevista.objects.filter(
        turma_prevista=turma_prevista)
    aluno_turma_prevista_candidato = aluno_turma_prevista.filter(
        status_aluno_turma_prevista=situacao_candidato)
    aluno_turma_prevista_matriculado = aluno_turma_prevista.filter(
        status_aluno_turma_prevista=situacao_matriculado)
    aluno_turma_prevista_nao_matriculado = aluno_turma_prevista.exclude(
        status_aluno_turma_prevista=situacao_candidato).exclude(status_aluno_turma_prevista=situacao_matriculado)

    data_matriculados = []
    for ta in aluno_turma_prevista_matriculado:
        temp_dict = {"nome": ta.aluno.nome,
                     "situacao": ta.status_aluno_turma_prevista.id, "aluno_id": ta.aluno.id}
        data_matriculados.append(temp_dict)
    data_matriculados = sorted(
        data_matriculados, key=lambda i: (i['situacao'], i['nome']))
    ...
    data_candidatos = sorted(
        data_candidatos, key=lambda i: (i['situacao'], i['nome']))
    ...
    data_nao_matriculados = sorted(
        data_nao_matriculados, key=lambda i: (i['situacao'], i['nome']))

    SituacaoFormset = formset_factory(
        form=Altera_Situacao_Prevista, formset=Altera_Situacao_Prevista_Formset, max_num=0)

    if request.method == "POST":
        post_data = request.POST.copy()
        formset_candidatos = SituacaoFormset(
            post_data, initial=data_candidatos, QUERYSET=situacoes_nao_matriculados, prefix="candidatos")
        formset_matriculados = SituacaoFormset(
            post_data, initial=data_matriculados, QUERYSET=Status_Aluno_Turma_Prevista.objects.all(), prefix="matriculados")
        formset_nao_matriculados = SituacaoFormset(
            post_data, initial=data_nao_matriculados, QUERYSET=situacoes_nao_matriculados, prefix="nao_matriculados")
        ...
        if formset_candidatos.is_valid() and formset_matriculados.is_valid() and formset_nao_matriculados.is_valid():
            for form in formset_candidatos:
                ...
                aluno_turma_temp = Aluno_Turma_Prevista.objects.get(
                    turma_prevista=turma_prevista, aluno=form.cleaned_data['aluno_id'])
    ...
    formset_candidatos = SituacaoFormset(
        QUERYSET=situacoes_nao_matriculados, initial=data_candidatos, prefix="candidatos")
    formset_matriculados = SituacaoFormset(QUERYSET=Status_Aluno_Turma_Prevista.objects.all(
    ), initial=data_matriculados, prefix="matriculados")
    formset_nao_matriculados = SituacaoFormset(
        QUERYSET=situacoes_nao_matriculados, initial=data_nao_matriculados, prefix="nao_matriculados")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if "_salvar" in post_data:
                ...
                #####
                aluno_turma = Aluno_Turma_Prevista.objects.filter(
                    aluno=aluno).exclude(turma_prevista=turma_prevista)
                aluno_turma = aluno_turma.filter(
                    status_aluno_turma_prevista=situacao_matriculado)
                for turma_aluno in aluno_turma:
                    ...
                    for horario in turma_aluno.turma_prevista.horario.all():
                        if horario in turma_prevista.horario.all():
                            messages.info(
                                request, 'Conflito de horário com a turma ' + str(turma_aluno.turma_prevista))
                            return HttpResponseRedirect(reverse('administracao:area_admin'))
                #####
...
#            return HttpResponseRedirect(reverse('administracao:area_admin'))
    ...
    for programa in programas:
        cursos_pgm = Curso.objects.filter(programa=programa).filter(
            exibir=True).filter(ativo=True)
        cursos.append({'programa': programa, 'cursos': cursos_pgm})

    form = CadForm(initial={'cidade': aluno.bairro.cidade,
                   'cpf': aluno.cpf}, instance=aluno)
...
def ConfirmarAluno(request, aluno_id, turma_id):
    ...
    aluno_turma_prevista = Aluno_Turma_Prevista.objects.get(
        aluno=aluno, turma_prevista=turma_prevista)
    status_confirma = Status_Aluno_Turma_Prevista.objects.get(
        descricao="Matriculado")

    if len(Turma_Prevista_Turma_Definitiva.objects.filter(turma_prevista=turma_prevista)) > 0:
        turma_definitiva_turma_prevista = Turma_Prevista_Turma_Definitiva.objects.get(
            turma_prevista=turma_prevista)
        aluno_turma, created = Aluno_Turma.objects.get_or_create(
            turma=turma_definitiva_turma_prevista.turma, aluno=aluno)
        aluno_turma.save()
...
def quantidade_situacao_aluno_turma(request):
    ...
    if request.method == 'POST':
        ...
        turmas = Turma.objects.filter(dt_inicio__gte=dt_inicio_i).filter(
            dt_inicio__lte=dt_inicio_f).filter(dt_fim__gte=dt_fim_i).filter(dt_fim__lte=dt_fim_f)
        ...
        for turma in turmas:
            ...
            for situacao in situacoes:
                lista_tmp.append(Aluno_Turma.objects.filter(
                    situacao=situacao, turma=turma).count())
...
def quantidade_situacao_aluno_turma_prevista(request):
    ...
    if request.method == 'POST':
        ...
        turmas_previstas = Turma_Prevista.objects.exclude(situacao='3').filter(dt_inicio__gte=dt_inicio_i).filter(
            dt_inicio__lte=dt_inicio_f).filter(dt_fim__gte=dt_fim_i).filter(dt_fim__lte=dt_fim_f)
        ...
        for turma_prevista in turmas_previstas:
            ...
            for status in status_aluno_turma_prevista:
                ...
                #            else:
                lista_tmp.append(Aluno_Turma_Prevista.objects.filter(
                    status_aluno_turma_prevista=status, turma_prevista=turma_prevista).count())
...
def alunos_formados_tel(request):
    ...
    alunos_turmas = Aluno_Turma.objects.filter(
        turma__dt_fim__lte='2019-07-14', situacao=situacao)
...
#        print ('aluno_turma: ', aluno_turma)
        ...
        for horario in turma.horario.all():
            ...
            temp_horario.append(dia + ' de ' + str(horario.hora_inicio)
                                [:5] + ' às ' + str(horario.hora_fim)[:5])
...
def encerrar_turma(request):
    if request.method == 'POST':
        ...
        alunos_turmas = Aluno_Turma.objects.filter(
            turma=turma_id).filter(situacao='1')
...
def total_cadastrados_em_dado_periodo(request):
    ...
    if request.method == 'POST':
        ...
        total = Aluno.objects.filter(dt_inclusao__gte=dt_inicio).filter(
            dt_inclusao__lte=dt_fim).count()
    ...
    for curso in cursos:
        ...
        for turma_definitiva in turmas_definitivas:
            total_alunos_definitivos += Aluno_Turma.objects.filter(
                turma=turma_definitiva, situacao__descricao = 'Confirmada').count()

        for turma_prevista in turmas_previstas:
            total_alunos_previstos += Aluno_Turma_Prevista.objects.filter(
                turma_prevista=turma_prevista, situacao__descricao = 'Aguardando').count()
...
def criar_curso(request):
    ...
    return render(request, 'Administracao/CRUD/cursos/criar.html', {'form': form})
...
def listar_cursos(request):
    ...
    return render(request, "Administracao/CRUD/cursos/listar.html", {"cursos": cursos})


@permission_required('cevest.acesso_admin', raise_exception=True)
@login_required
def exibir_curso(request, pk):

    curso = get_object_or_404(Curso, pk=pk)

    return render(request, 'Administracao/CRUD/cursos/exibir.html', {'curso': curso})
...
def editar_curso(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('administracao:exibir_curso', pk=curso.pk)
        else:
            erro_tmp = str(form.errors)
            messages.error(request, erro_tmp[1] + ': ' + erro_tmp[2])
    else:
    ...
    return render(request, 'Administracao/CRUD/cursos/editar.html', {'form': form, 'curso': curso})
