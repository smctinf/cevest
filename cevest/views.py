...
def pauta(request):
    ...
    if request.user.is_superuser or request.user.has_perm('Administracao.pode_emitir_certificado'):
        ...
        turmas = Turma.objects.filter(
            instrutor=instrutor).filter(dt_fechamento=None)
...
def pauta2(request, turma_id):
    ...
    alunos = Aluno_Turma.objects.order_by(
        'aluno').filter(turma=turma_id).filter(situacao=1)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            aluno = Aluno.objects.get(
                cpf='05334010700', dt_nascimento='1978-11-02')
            if aluno != 'None':
                pk = aluno.pk
#                form = AlteraForm()
...
def turma_prevista(request, idcurso):
    turmas_previstas = Turma_Prevista.objects.filter(
        curso=idcurso, exibir=True, dt_fim__gt=datetime.date.today())
...
def portador(request):
    if request.user.is_authenticated:
        portador = Aluno.objects.filter(
            portador_necessidades_especiais=True).order_by('nome')
...
def altera_cpf(request):
    ...
    if request.method == 'POST':
        ...
        if altera_form.is_valid():
            ...
            try:
                aluno_temp = Aluno.objects.get(
                    cpf=cpf_temp, dt_nascimento=nasc_temp)
...
def AlterarCadastro(request):
    ...
    #
    form = CadFormBase(
        initial={'cidade': aluno_temp.bairro.cidade}, instance=aluno_temp)
...
def alocados(request):
    if request.user.is_authenticated:
        ...
        necessidades = Aluno_Turma.objects.raw(
            'select cevest_aluno.id as aluno_id, cevest_turma_prevista.id as turma_id from cevest_aluno, cevest_turma_prevista, cevest_aluno_cursos where cevest_aluno, cevest_turma_prevista.id and cevest_aluno_cursos.curso_id = cursos.id and cevest_aluno.cursos_id = cursos.id')
...
def getLista_Candidatos():
    ...
    turmas_previstas_remover = Turma_Prevista.objects.filter(
        situacao=situacao_cancelada)
    turmas_previstas_remover = turmas_previstas_remover.filter(
    ...
    aluno_turma = Aluno_Turma_Prevista.objects.filter(
        status_aluno_turma_prevista=status).order_by("aluno")
...
def getLista_Alocados():
    ...
    desistiu_da_vaga = Status_Aluno_Turma_Prevista.objects.get(
        descricao='Desistiu da Vaga')
    nao_compareceu = Status_Aluno_Turma_Prevista.objects.get(
    ...
    for turma in turmas_previstas:
        temp_lista_alunos = []
        temp_lista_horarios = []
        temp_turma_aluno = Aluno_Turma_Prevista.objects.filter(turma_prevista=turma).exclude(
            status_aluno_turma_prevista=desistiu_da_vaga).exclude(status_aluno_turma_prevista=nao_compareceu)
        ...
        lista_turmas.append(
            {"turma": turma, "alunos": temp_lista_alunos, "horarios": temp_lista_horarios})
    return lista_turmas
...
def getTotalConcluidosPorCurso():
    ...
    for curso in cursos:
        ...
        temp_total = 0
        for turma in turmas:
            temp_total += Aluno_Turma.objects.filter(
                turma=turma, situacao=situacao_aprovado).count()
...
def getInteresseTotalPorCursoETurno():
    ...
    for curso in cursos:
        lista_temp_cursos.append(
            {'Curso': curso, manha: 0, tarde: 0, noite: 0})
